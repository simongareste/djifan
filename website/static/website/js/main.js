...
var respUtils = window.WEBSITE_LIB.importResponsiveUtilities();
var mobileMenu = window.WEBSITE_LIB.importMobileMenu();

$(document).ready(function(){
  mobileMenu.setup({
    menuSelector: ".nav",
    navSelector: ".menu_wrap",
    afterHide: renderGallery
  });
  ...
  if (!$('body').hasClass('gallery')) {
    if (respUtils.tablet() && WEBSITE_CONF.theme.gallery_image_height != 'Full Browser Height') {
...
function positionContent() {
  ...
  if($(window).height() < menu_height + 10) {
    ...
    if (isDevice && WEBSITE_CONF.theme.gallery_image_height === 'Full Browser Height') {
    ...
    if (isDevice) {
      if (WEBSITE_CONF.theme.gallery_image_height === 'Full Browser Height') {
        var menuWrapHeight = $(window).height() - $('#logo').outerHeight() - $('.navbar > .social_icons').outerHeight() - 20;
      } else {
...
function initGallery() {
  ...
  if(WEBSITE_CONF.theme.nav_arrows_toggle) {
    ...
    if(WEBSITE_CONF.theme.nav_arrow_thickness === 'Thin'){
      content.addClass('cursor_s')
    } else if(WEBSITE_CONF.theme.nav_arrow_thickness === 'Medium'){
      content.addClass('cursor_m')
    } else if(WEBSITE_CONF.theme.nav_arrow_thickness === 'Thick'){
      content.addClass('cursor_l')
    }

    if(WEBSITE_CONF.theme.nav_arrow_style === 'Light'){
      content.addClass('cursor_white')
    } else if(WEBSITE_CONF.theme.nav_arrow_style === 'Dark'){
  ...
  if( !isDevice && WEBSITE_CONF.theme.gallery_image_height != 'Full Browser Height'){
    WEBSITE_LIB.Lazyload.add('.asset img', {
      priority: 10,
...
function initListing() {
  WEBSITE_LIB.Lazyload.add('.asset .img img', {
...
function moveSlider(direction) {   // slide index
  ...
  if(WEBSITE_CONF.theme.gallery_change_image_speed == 'Slow') {
    imageSpeed = 800;
  } else if(WEBSITE_CONF.theme.gallery_change_image_speed == 'Normal') {
    imageSpeed = 550;
  } else if(WEBSITE_CONF.theme.gallery_change_image_speed == 'Fast') {
    imageSpeed = 400;
  }
  ...
  try {
    ...
    var activeSlidePosition = parseInt(target_slide.position().left, 10) + target_slide.outerWidth()/2 - $(window).width()/2;
    ...
    if (activeSlidePosition < 0 && $window.scrollLeft() == 0) {
      ...
      $('body, html').stop().animate({ scrollLeft: activeSlidePosition }, {queue :false, duration: imageSpeed, complete: onScrollComplete });
    }
  } catch (err) {
    var slide_direction = direction > 0 ? '+=350' : '-=350';
    $('body, html').stop().animate({scrollLeft: slide_direction }, {
      queue: false,
      duration: imageSpeed,
      complete: function() {
        activeSlideIndex = parseInt(location.hash.slice(1));
...
function setActiveSlide() {
  ...
  var $slides = $('#content .asset');
	var $window = $(window);
	var scrollLeft = $("body")[0].scrollLeft > $("html")[0].scrollLeft ? $("body")[0].scrollLeft : $("html")[0].scrollLeft;
	var assetIndex = 0;
	var $asset = null;

	if( scrollLeft <= ( $slides.first().width() / 2) ){
		assetIndex = 0;
		$asset = $slides.first();
		activeSlideIndex = assetIndex;
		setLocationHash(assetIndex);
	} else if ( scrollLeft >= ( $slides.last().offset().left - $slides.last().width() / 2) ){
...
function customCursor(e) {
  ...
  if(activeSlideIndex == 0) { content.addClass('cursor_right').removeClass('cursor_left'); }
  else if(activeSlideIndex == slideAssetsLength) { content.addClass('cursor_left').removeClass('cursor_right'); }
}
